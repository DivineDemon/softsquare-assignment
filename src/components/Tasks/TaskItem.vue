<template>
  <div class="m-2 p-2 flex flex-col space-y-2 bg-white rounded-md">
      ...
      <div class="flex flex-row space-x-3 relative">
        ...
        >
          <p class="flex flex-row space-x-3 items-center">
            ...
            <span
              ><router-link :to="`/edit/${task.id}`">Edit</router-link></span
            >
          </p>
          <p class="flex flex-row space-x-3 items-center cursor-pointer">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="currentColor"
    ...
    <p class="text-md font-semibold text-gray-600 capitalize">
      {{ task.description }}
    </p>
  </div>
</template>
